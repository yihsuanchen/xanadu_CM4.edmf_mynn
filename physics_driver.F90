...
!#######################################################################
  subroutine compute_vert_adv_tend (is, ie, js, je, Time_next, &
    ...
    !--- running AM4 3D run, vert_advection will return this error message: 
    !    FATAL: STDLOG: is called inside a OMP parallel region
    !    So, I move vert_advection subroutine here, instead of calling from vert_advection_mod
    !use vert_advection_mod,       only: vert_advection, SECOND_CENTERED, &
...
!write(6,*) 'ggaa, omega_h', omega_h
    ...
    select case (temp_vert_advec_scheme)
     case(1)
        call vert_advection_3d_copy(dts,omega_h,dp,pt,dT_vadv,scheme=SECOND_CENTERED,form=ADVECTIVE_FORM)
     case(2)
        call vert_advection_3d_copy(dts,omega_h,dp,pt,dT_vadv,scheme=FOURTH_CENTERED,form=ADVECTIVE_FORM)
     case(3)
        call vert_advection_3d_copy(dts,omega_h,dp,pt,dT_vadv,scheme=FINITE_VOLUME_LINEAR,form=ADVECTIVE_FORM)
     case(4)
        call vert_advection_3d_copy(dts,omega_h,dp,pt,dT_vadv,scheme=FINITE_VOLUME_PARABOLIC,form=ADVECTIVE_FORM)
     case(5)
        call vert_advection_3d_copy(dts,omega_h,dp,pt,dT_vadv,scheme=SECOND_CENTERED_WTS,form=ADVECTIVE_FORM)
     case(6)
        call vert_advection_3d_copy(dts,omega_h,dp,pt,dT_vadv,scheme=FOURTH_CENTERED_WTS,form=ADVECTIVE_FORM)
     end select
    ...
    !--- compute tracer vertical advection
     SELECT CASE (tracer_vert_advec_scheme)
            ...
            CASE(6)
                 vadvec_scheme = FOURTH_CENTERED_WTS
     END SELECT
     call vert_advection_3d_copy(dts,omega_h,dp,qq,dqv_vadv,scheme=vadvec_scheme,form=ADVECTIVE_FORM)
...
!<--- yhc, 2023-01-13
!     copy subroutine vert_advection from vert_advection_mod

 subroutine vert_advection_3d_copy ( dt, w, dz, r, rdt, mask, scheme, form, flags )
    ...
    use vert_advection_mod,       only: SECOND_CENTERED, &
                                        ...
                                        SECOND_CENTERED_WTS, FOURTH_CENTERED_WTS, &
                                        ADVECTIVE_FORM, &
                                        VAN_LEER_LINEAR, FLUX_FORM, FINITE_VOLUME_PARABOLIC, FINITE_VOLUME_PARABOLIC2   ! yhc add
...
! for cfl diagnostics with finite volume schemes
 ...
 end subroutine vert_advection_3d_copy
...
! compute slope (weighted for unequal levels)
 ...
 real, dimension(size(dz,1),size(dz,2),size(dz,3)) :: dzs    ! yhc add
 real, dimension(0:3, size(dz,1),size(dz,2),size(dz,3)) :: zwts
