...
#include <fms_platform.h>
  ...
  use      Constants_Mod, ONLY: tfreeze,HLv,HLf,HLs,CP_AIR,GRAV,Kappa,rdgas,rvgas,vonkarm
...
!-------  interfaces --------
  ...
  NAMELIST / uw_conv_nml / iclosure, rkm_sh1, rkm_sh, cldhgt_max, plev_cin, plev_omg, eis_max,eis_min,do_peff_land, &
       ...
       wmin_ratio,  landfact_m, pblht0, lofactor0, lochoice, do_downdraft, do_shallow, &
       ...
       do_eis_limit, do_eis_limitn, do_lts_limit, do_lts_limitn, do_cmf_pblzero, compute_pblht, parcel_buoy, temp_jump
...
!------------------------------------------------------------------------
  ...
  integer :: id_tdt_uwc, id_qdt_uwc, id_udt_uwc, id_vdt_uwc, id_prec_uwc, id_snow_uwc, &
       ...
       id_dbuodp_uws, id_dbuodp_uwd, id_dthvdp_uwc, id_dqsdp_adi,          &
       ...
       id_zsml_uwc, id_hlsrc_avg, id_qtsrc_avg, id_cape_avg, id_cin_avg, id_omg_avg,       &
...
!========Option for idealized forcing=====================================
    ...
    id_dqsdp_adi = register_diag_field ( mod_name, 'dqsdp_adi', axes(1:3), Time,   &
         'dqsdp_adi', 'kg/kg/Pa', missing_value=mv)
  ...
  if(do_mse_budget) then
    ...
    id_zsml_uwc= register_diag_field ( mod_name,'zsml_uwc', axes(1:2), Time, &
         'PBL height computed in uw_conv', 'm' )
...
!CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
    ...
    real, intent(in),    dimension(:,:)  :: ustar, bstar, qstar,  lat, lon !pbl height...
    real, intent(inout), dimension(:,:)  :: pblht, cush  ! convective scale height (m)
...
!required when do_mse_budget=T
    ...
    real, dimension(size(tb,1),size(tb,2)) :: &
         ...
         shflx,      &
         cltc,       &
         hfint0_old,  omg_avg, hlsrc_avg, qtsrc_avg, cape_avg, cin_avg, &
...
!commented out when do_mse_buget=T
    ...
    real, dimension(size(tb,1),size(tb,2),size(tb,3)) :: dqsdp_adi
...
!
 ...
 !========Option for deep convection=======================================
    ...
    real,    dimension(size(tb,1),size(tb,2),size(tb,3)) ::   zfull_ag, slv, hleff
    real,    dimension(size(tb,1),size(tb,2),size(tb,3)+1) :: zhalf_ag
    real,    dimension(size(tb,1),size(tb,2)) :: zsml, zsurf
!    integer, dimension(size(tb,1),size(tb,2)) :: kbot
    
    integer imax, jmax, kmax, nlev, ipbl, ibot
    integer kd, ntracers
...
!commented out when do_mse_budget=T
    ...
    omg_avg=0.; hlsrc_avg=0.; qtsrc_avg=0.; cape_avg=0.; cin_avg=0.;
    qldet_s=0.; qidet_s=0.; qadet_s=0.; qndet_s=0.; dqsdp_adi=0.;
...
!========Option for deep convection=======================================
    ...
    if (compute_pblht) then
       nlev = kmax 
       zsurf(:,:) = zint(:,:,nlev+1)
       do k = 1, nlev
          zfull_ag(:,:,k) = zmid(:,:,k) - zsurf(:,:)
          zhalf_ag(:,:,k) = zint(:,:,k) - zsurf(:,:)
       enddo
       zhalf_ag(:,:,nlev+1) = zint(:,:,nlev+1) - zsurf(:,:)
       hleff = (min(1.,max(0.,0.05*(tb      -tfreeze+20.)))*hlv + &
              min(1.,max(0.,0.05*(tfreeze -tb         )))*hls)
       slv   = cp_air*tb + grav*zfull_ag - hleff*(qtr(:,:,:,nql) + qtr(:,:,:,nqi))
       slv   = slv*(1+d608*(qv + qtr(:,:,:,nql) + qtr(:,:,:,nqi)))
       ibot = nlev
       ...
       do i=1,imax
          do j=1,jmax
             ipbl    = -1
             if (bstar(i,j) .gt. 0.) then
                call pbl_depth(slv(i,j,1:ibot)/cp_air, &
                     zfull_ag(i,j,1:ibot),ustar(i,j),bstar(i,j),parcel_buoy, &
                     ipbl,zsml(i,j))
         !------------------------------------------------------
...
!========Find source air, and do adiabatic cloud lifting======================
          ...
          do k = 1,kmax
             ...
             dqsdp_adi(i,j,nk) = ac%dqsdp_adi(k);
          end do
...
!========Cumulus closure to determine cloud base mass flux===================
          ...
          if (do_shallow .eq. .false.) then
             ocode(i,j)=100; cbmf_shallow=0.; goto 100
          end if
          if (ac%klcl.eq.0 .or. ac%plcl.eq.sd%ps(1) .or. ac%plcl.lt.20000.) then
             ocode(i,j)=1; cbmf_shallow=0.; goto 100
          end if
...
!scaling factor for each column is the minimum value within that column
    ...
    if (do_cmf_pblzero) then
       do k = 1,kmax
          do j = 1,jmax
             do i = 1,imax
                if (zmid(i,j,k) .le. pblht(i,j)) then
                   cmf(i,j,k) = 0
                end if
             enddo
          enddo
...
!      used = send_data( id_tten_rad_uwc,tten_rad*aday,Time, is, js, 1)
    ...
    used = send_data( id_zsml_uwc,         zsml,               Time, is, js)
   ...
   ! cmip diagnostics
    ...
    used = send_data( id_dqsdp_adi,  dqsdp_adi,    Time, is, js, 1)
...
!!<--- yhc 2021-12-09 write out variables
!        write(6,*)    ';======== input argument ============' 
!        write(6,*)    '; height@model interfaces(m)'
!        write(6,3002) '  zw = (/'    ,zint
!        write(6,*)    '' 
!        write(6,*)    '; height@model mid-levels (m)'
!        write(6,3002) '  zfull = (/'    ,zmid
!        write(6,*)    ''
!        write(6,*)    '; temperature profile (K)'
!        write(6,3002) '  tb = (/'    , tb
!        write(6,*)    ''
!        write(6,*)    '; specific humidity profile (kg/kg)'
!        write(6,3002) '  qv = (/'    , qv
!        write(6,*)    ''
!        write(6,*)    '; pressure@model mid-levels (pa)'
!        write(6,3002) '  pmid = (/'    , pmid
!        write(6,*)    ''
!        write(6,*)    '; input liq_wat'
!        write(6,3002) ' ql = (/'    ,qtr(1,1,:,nql)
!        write(6,*)    ''
!        write(6,*)    '; input ice_wat'
!        write(6,3002) ' qi = (/'    ,qtr(1,1,:,nqi)
!        write(6,*)    ''
!        write(6,*)    '; input cld_amt'
!        write(6,3002) '  cldfra_bl1d = (/'    ,qtr(1,1,:,nqa)
!        write(6,*)    ''
!        write(6,*)    ';------ cmf -----'
!        write(6,*)    '; cmf_uws '
!        write(6,3002) '  cmf_uws = (/'    , cmf_s
!        write(6,*)    ''
!        write(6,*)    '; cmf_uwd '
!        write(6,3002) '  cmf_uwd = (/'    , cmf_d
!        write(6,*)    ''
!        write(6,*)    ';======== output argument from uw_conv ============' 
!        write(6,*)    ''
!        write(6,*)    ';------ qa -----'
!        write(6,*)    '; qadt_uws '
!        write(6,3002) '  qadt_uws = (/'    , qaten-qaten_d
!        write(6,*)    ''
!        write(6,*)    '; qadet_uws'
!        write(6,3002) '  qadet_uws = (/'    , qadet_s
!        write(6,*)    ''
!        write(6,*)    '; qasub_uws'
!        write(6,3002) '  qasub_uws = (/'    , qaten-qaten_d-qadet_s
!        write(6,*)    ''
!        write(6,*)    '; qadt_uwd'
!        write(6,3002) '  qadt_uwd = (/'    ,  qaten_d
!        write(6,*)    ''
!        write(6,*)    '; qadet_uwd'
!        write(6,3002) '  qadet_uwd = (/'    , qadet_d
!        write(6,*)    ''
!        write(6,*)    '; qasub_uwd'
!        write(6,3002) '  qasub_uwd = (/'    , qaten_d-qadet_d
!        write(6,*)    ''
!        write(6,*)    ';------ ql -----'
!        write(6,*)    '; qldt_uws '
!        write(6,3002) '  qldt_uws = (/'    , qlten-qlten_d
!        write(6,*)    ''
!        write(6,*)    '; qldet_uws'
!        write(6,3002) '  qldet_uws = (/'    , qldet_s
!        write(6,*)    ''
!        write(6,*)    '; qlsub_uws'
!        write(6,3002) '  qlsub_uws = (/'    , qlten-qlten_d-qldet_s
!        write(6,*)    ''
!        write(6,*)    '; qldt_uwd'
!        write(6,3002) '  qldt_uwd = (/'    ,  qlten_d
!        write(6,*)    ''
!        write(6,*)    '; qldet_uwd'
!        write(6,3002) '  qldet_uwd = (/'    , qldet_d
!        write(6,*)    ''
!        write(6,*)    '; qlsub_uwd'
!        write(6,3002) '  qlsub_uwd = (/'    , qlten_d-qldet_d
!        write(6,*)    ''
!        write(6,*)    ';------ qi -----'
!        write(6,*)    '; qidt_uws '
!        write(6,3002) '  qidt_uws = (/'    , qiten-qiten_d
!        write(6,*)    ''
!        write(6,*)    '; qidet_uws'
!        write(6,3002) '  qidet_uws = (/'    , qidet_s
!        write(6,*)    ''
!        write(6,*)    '; qisub_uws'
!        write(6,3002) '  qisub_uws = (/'    , qiten-qiten_d-qidet_s
!        write(6,*)    ''
!        write(6,*)    '; qidt_uwd'
!        write(6,3002) '  qidt_uwd = (/'    ,  qiten_d
!        write(6,*)    ''
!        write(6,*)    '; qidet_uwd'
!        write(6,3002) '  qidet_uwd = (/'    , qidet_d
!        write(6,*)    ''
!        write(6,*)    '; qisub_uwd'
!        write(6,3002) '  qisub_uwd = (/'    , qiten_d-qidet_d
!        write(6,*)    ''
!        !write(6,*)    '; '
!        !write(6,3002) '  = (/'    ,
!        !write(6,*)    ''
...
subroutine pbl_depth(t, z, u_star, b_star, parcel_buoy, ipbl, h)  ! From Lock scheme
...
real,    intent(in)                 :: u_star, b_star, parcel_buoy
